...
- Steamライブラリに眠る積みゲーを金額換算して可視化し、
  積みゲーの消化促進、および積みがちなゲーム傾向を示すことで購買行動の最適化を目的としています。

- 誰の: 可処分所得・時間が限られる社会人ゲーマー
- どんな課題を: 積みゲーによる罪悪感と選択肢疲れ
- どう解決するか: 金額で可視化 + 次にプレイするゲームを提示 + キャラとの交流で消化を促進
...
- この罪悪感に強制的に目を向けさせつつ、積みゲーを消化し、解消するための動機付けを行ってくれる外部アプリを作成したいと考えました。
...
- したがって、積みゲーを消化する動機づけ強化のために購入総額を可視化し、ある種の痛みとしての自覚に加えて、
  積みゲーを消化したこと自体に対する外部的な報酬を設定することで「消化したい」という気持ちを強化できるとよいと考えました。
...
- 自分自身
- 理由：自分自身、積みゲーからくる罪悪感や選択肢疲れにより不快感を感じているため

---

### 4-3. 実際に使われる可能性について(需要・存在確認)

- 類似のサービスが存在しており、需要は存在すると考えています。
...
- 消化によってアプリ内ポイントを獲得し、キャラクターとの交流や着せ替えが可能にする
...
- また、オリジナルキャラクターによる罪悪感の軽減と、そのキャラクターとの交流という積みゲー消化を促す機能を実装したいと考えています。
...
### MVPで作る機能
- SteamworksAPIを使用したライブラリおよびゲーム定価の取得
...
- オリジナルキャラクターのとの交流、および着せ替え機能の実装が継続的にご利用いただく動機付けとして働くことを期待しています。
